...
class DBProvider {
  ...
  Future _insertContact(EncodableContact c, Hangout hangout) async {
    final db = await database;
    return db.rawInsert(
        "INSERT INTO contacts (birthday,displayName,familyName,middleName,givenName,identifier,hangoutId,avatar)"
        " VALUES (?,?,?,?,?,?,?,?)",
        [
          c.birthday != null ? c.birthday.toIso8601String() : null,
          c.displayName,
          c.familyName,
          c.middleName,
          c.givenName,
  ...
  Future _updateContact(EncodableContact c, Hangout hangout) async {
    final db = await database;
    return db.rawUpdate(
        "UPDATE contacts SET birthday = ?, displayName = ?, familyName = ?, middleName = ?, givenName = ?, identifier = ?, hangoutId = ?, avatar = ?"
