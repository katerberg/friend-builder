...
///
/// Native owns templates and `tel:` open; Dart owns ranking, photos, phones,
/// and hangout writes.
class CarPlayService {
  ...
  static Future<Map<String, dynamic>> getTopPersonPayload() async {
    try {
      ...
      final basePayload = result.toSnapshotPayload();
      if (basePayload['found'] != true) {
        return basePayload;
      }

      final contactIdentifier = basePayload['contactIdentifier'] as String;
      ...
      final phones = matchedContact == null
          ? <Map<String, String>>[]
          : carPlayPhonesFromContact(matchedContact);
      final photoBytes =
          await ContactPermissionService.getContactPhoto(contactIdentifier);
